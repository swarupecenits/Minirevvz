...
interface FilterSidebarProps {
  ...
  setShowPremiumOnly: (val: boolean) => void;
  showNewOnly: boolean;
      ...
      <div>
        ...
        <div className="space-y-3">
          <label className="flex items-center gap-3 cursor-pointer group">
            ...
            >
              {showPremiumOnly && <div className="w-2.5 h-2.5 bg-zinc-950 rounded-sm" />}
            </div>
          ...
          <label className="flex items-center gap-3 cursor-pointer group">
            ...
            >
              {showNewOnly && <div className="w-2.5 h-2.5 bg-zinc-950 rounded-sm" />}
            </div>
...
export function Products() {
  ...
  const filterSidebarProps = useMemo(() => ({
    ...
    showNewOnly,
    setShowNewOnly
  }), [selectedCategory, showPremiumOnly, showNewOnly, setSelectedCategory, setShowPremiumOnly, setShowNewOnly]);

  return (
    <div className="pt-24 pb-24 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 min-h-screen">
      <div className="flex flex-col md:flex-row md:items-end justify-between mb-12 gap-6">
        <div>
          <h1 className="text-3xl sm:text-4xl font-display font-bold text-zinc-100 mb-2">
      ...
      <div className="flex flex-col md:flex-row gap-12">
        ...
        <div className="flex-1">
          {filteredAndSortedProducts.length > 0 ? (
            <motion.div
              layout
              className="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-3 gap-6 px-2 sm:px-0"
            >
